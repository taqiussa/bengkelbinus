<div>
        ...
        <x-slot name="body">
            @foreach ($services as $key => $service)
                <tr x-data="window.__controller.dataTableController({{ $service->id }})">
                    ...
                    <td><a role="button" wire:click="edit({{ $service->id }})" class="mr-3 btn btn-danger">{{ $service->status }}</a></td>
                    <td>{{ $service->created_at->format('d M Y H:i') }}</td>
                    <td class="whitespace-no-wrap row-action--icon">
                        <a role="button" wire:click="editpayment({{ $service->id }})" class="mr-3 btn btn-danger">Check Out</a>
    ...
    @if ($isOpen)
    @include('modal.modal-servicedone')
    @endif
    @if ($isOpens)
    @include('modal.modal-payment')
    @endif   
</div>
